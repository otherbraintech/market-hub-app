      ...
      {!isLoading && results && (
        <div className="space-y-6">
          ...
          <Tabs defaultValue="hooks" className="space-y-6">
            <TabsList className="grid grid-cols-2 md:grid-cols-7 h-auto p-1.5 bg-muted/60 rounded-xl gap-1">
              ...
              <TabsTrigger value="prompt" className="py-2.5 text-xs font-bold gap-1.5">
                <Code className="h-3.5 w-3.5 text-amber-500" /> System Prompt
              </TabsTrigger>
            </TabsList>
              ...
              {results.hooks && results.hooks.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {results.hooks.map((hk: any, idx: number) => (
                    <Card key={idx} className="border border-border/60 hover:border-orange-500/40 transition-all bg-card/80">
                      ...
                      <CardContent className="space-y-3 text-xs md:text-sm">
                        ...
                        {hk.suggested_cta && (
                          <div className="flex items-center gap-2 pt-2 border-t border-border/40 text-xs font-semibold text-cyan-600 dark:text-cyan-400">
                            <span>CTA Sugerido:</span>
                            <span className="bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20 font-bold">{hk.suggested_cta}</span>
                          </div>
                        )}
                      </CardContent>
                    </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div key={i} className="p-3 rounded-lg bg-muted/50 space-y-1 text-xs">
                          <div className="flex items-center justify-between">
                            ...
                            <Badge variant="outline" className="bg-purple-500/10 text-purple-600 border-purple-500/20 text-[10px] uppercase font-bold">
                              {m.audio_phase || m.lifecycle_status || "pico"}
                            </Badge>
                          </div>
                ...
                <Card className="border border-cyan-500/20">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">OpenRouter GPT-5.6-sol Engine</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                      ...
                      results.generalMusic.map((gm: any, i: number) => (
                        <div key={i} className="p-2.5 rounded-lg bg-muted/50 flex items-center justify-between text-xs">
                          <div>
                            ...
                            <div className="text-muted-foreground text-[11px]">{gm.artist || "Global Trending"}</div>
                          </div>
                          <Badge className="bg-cyan-500/10 text-cyan-600 text-[10px]">#TopViral</Badge>
                        </div>
                      ))
                    ) : (
                ...
                <Card className="border border-emerald-500/30 bg-emerald-500/5">
                  ...
                  <CardContent className="space-y-2">
                    <div className="text-2xl font-black">
                      {results.algorithmicRecommendations?.queen_metric?.name || "Rewatch Rate"}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Objetivo: <strong className="text-foreground">{results.algorithmicRecommendations?.queen_metric?.target || ">2.0 loops por usuario"}</strong>
                    </p>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {results.contentPillars.map((p: any, i: number) => (
                        <div key={i} className="p-4 rounded-xl bg-muted/50 space-y-2 border border-border/40">
                      ...
                      <Card key={i} className="border border-border/60">
                        <CardHeader className="pb-2">
                          <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                            ...
                            <span>{nw.published_at || "Reciente"}</span>
                          </div>
            ...
            {/* TAB 7: System Prompt Generado */}
            <TabsContent value="prompt" className="space-y-4">
              <Card className="border border-amber-500/30 bg-black/90 text-white font-mono">
                <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-neutral-800">
                  <div>
                    <CardTitle className="text-sm font-bold text-amber-400 flex items-center gap-2">
                      <Code className="h-4 w-4" /> System Prompt Inyectable para LLMs (GPT-4o / Claude / Gemini)
                    </CardTitle>
                    <CardDescription className="text-xs text-neutral-400">
                      Sintetizado según la especificación oficial de OB-Tendencias API Engine 2026.4
                    </CardDescription>
                  </div>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => copyToClipboard(results.systemPrompt)}
                    className="border-neutral-700 text-neutral-200 hover:bg-neutral-800"
                  >
                    {copiedPrompt ? <Check className="h-4 w-4 text-emerald-400 mr-1.5" /> : <Copy className="h-4 w-4 mr-1.5" />}
                    {copiedPrompt ? "Copiado" : "Copiar Prompt"}
                  </Button>
                </CardHeader>
                <CardContent className="p-4 overflow-x-auto text-xs leading-relaxed text-neutral-300">
                  <pre className="whitespace-pre-wrap">{results.systemPrompt}</pre>
                </CardContent>
              </Card>
            </TabsContent>
          </Tabs>
