...
export default async function BusinessPage() {
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 max-w-[1600px] mx-auto animate-fade-in text-slate-100">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-[#0D1526] p-8 rounded-2xl border border-cyan-500/15 shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="flex flex-col gap-2 min-w-[220px]">
            <div className="flex justify-between items-end w-full text-[10.5px] font-extrabold uppercase tracking-widest text-slate-400">
              <span>Capacidad del Plan</span>
              <span className={`text-xs ${count >= limit ? "text-rose-400 font-bold" : "text-cyan-400 font-black"}`}>
                {count} / {limit} Clientes
              </span>
            </div>
            <Progress value={percentage} className="h-2 w-full bg-slate-900 shadow-inner" />
          </div>
          
          {count > 0 && <div className="h-10 w-px bg-slate-800 hidden sm:block" />}
          
          {count === 0 ? null : count < limit ? (
            <CreateBusinessDialog />
          ) : (
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-900 rounded-xl text-xs font-bold text-slate-400 border border-slate-800">
              <ShieldCheck className="h-4 w-4 text-amber-400" /> Plan al límite
            </div>
          )}
        </div>
      </div>
